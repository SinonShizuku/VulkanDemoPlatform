...
**材质槽位盘点（2026-09-13，加载诊断）**：loader 目前只消费 base color 槽，其余槽位只统计。三个 Bistro 场景实测（`--frames 2 --warmup 0`，日志里的 `[ Model ] slots:` 一行）：

| 场景 | 材质数 | base(diffuse) | normal | specular | emissive | metallic / roughness / AO |
| --- | --- | --- | --- | --- | --- | --- |
| `BistroExterior.fbx` | 132 | 132（全部加载，skipped=0） | 132（未消费） | 131（未消费） | 10（未消费） | 0 / 0 / 0 |
| `BistroInterior.fbx` | 71 | 71（全部加载） | 71 | 70 | 4 | 0 / 0 / 0 |
| `BistroInterior_Wine.fbx` | 74 | 68（6 个材质本身没有 base color 贴图） | 68 | 67 | 4 | 0 / 0 / 0 |

两个结论直接影响 PBR 阶段的做法：

1. assimp 把 Bistro 的 base color 报成 `aiTextureType_DIFFUSE` 而**不是** `aiTextureType_BASE_COLOR`——"BASE_COLOR 优先、回落到 DIFFUSE"的写法是必需的（否则一张贴图都加载不到）；
2. 这批资产是 **spec/gloss 工作流**（metalness / roughness / AO 槽位全为 0）：PBR 阶段要么直接实现 spec/gloss，要么做一次 spec→metal 转换（roughness 通常在 specular 贴图的 alpha 里，BC3 编码保留 alpha；BC1 没有 alpha）。normal / specular / emissive 的文件格式是 BC5/BC1/BC3，**我们的 DDS 解码器都已经支持**，接线即可。
